/// <reference types="npm:@types/react@18.3.1" />
import * as React from "npm:react@18.3.1";
import {
  Html, Head, Body, Container, Section, Text, Button, Img, Hr, Preview,
} from "npm:@react-email/components@0.0.22";
import {
  LOGO_URL, main, container, logoSection, logoImg, heroSection, heroEmoji,
  heroTitle, heroSubtitle, ctaSection, ctaButton, footerDivider,
  footerSection, footerText, linkStyle, textBody, SITE_URL,
} from "./_styles.ts";

interface Props {
  userName: string;
  planName: string;
  role: string;
  siteUrl?: string;
}

export function SubscriptionWelcome({
  userName = "Usuário",
  planName = "Pro",
  role = "gm",
  siteUrl = SITE_URL,
}: Props) {
  const dashboardUrl =
    role === "gm" ? `${siteUrl}/dashboard/mestre`
    : role === "store" ? `${siteUrl}/dashboard/loja`
    : `${siteUrl}/dashboard/jogador`;

  return (
    <Html><Head />
      <Preview>Bem-vindo ao plano {planName} do HIVIUM! 🎉</Preview>
      <Body style={main}>
        <Container style={container}>
          <Section style={logoSection}>
            <Img src={LOGO_URL} width="56" height="56" alt="HIVIUM" style={logoImg} />
          </Section>
          <Section style={heroSection}>
            <Text style={heroEmoji}>🏆</Text>
            <Text style={heroTitle}>Bem-vindo ao {planName}!</Text>
            <Text style={heroSubtitle}>{userName}, sua assinatura está ativa. Hora de desbloquear todo o potencial da plataforma.</Text>
          </Section>
          <Section>
            <Text style={textBody}>
              Com seu novo plano, você tem acesso a recursos exclusivos que vão turbinar sua experiência na HIVIUM. Explore seu dashboard para ver tudo que está disponível.
            </Text>
          </Section>
          <Section style={ctaSection}>
            <Button style={ctaButton} href={dashboardUrl}>Acessar meu dashboard</Button>
          </Section>
          <Hr style={footerDivider} />
          <Section style={footerSection}>
            <Text style={footerText}>HIVIUM — A plataforma que conecta sua mesa ao jogador certo.</Text>
            <Text style={{ fontSize: "12px", margin: "0" }}><a href={siteUrl} style={linkStyle}>hivium.app</a></Text>
          </Section>
        </Container>
      </Body>
    </Html>
  );
}
