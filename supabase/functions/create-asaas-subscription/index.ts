...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch first payment generated by subscription to get PIX QR code
    let pixData: { encodedImage?: string; payload?: string; expirationDate?: string } = {};
    let firstPayment: { id?: string; status?: string; invoiceUrl?: string; bankSlipUrl?: string } = {};
    ...
    try {
      ...
      const paymentsRes = await fetch(
        `${ASAAS_BASE}/payments?subscription=${asaasSub.id}&limit=1&order=asc`,
      ...
      if (paymentsRes.ok) {
        ...
        if (paymentsData.data?.length > 0) {
          ...
          log("First payment found", { paymentId: firstPayment.id, status: firstPayment.status });
          ...
          if (billing_type === "PIX" && firstPayment.id) {
            ...
            if (pixRes.ok) {
              ...
              log("PIX QR code retrieved", { hasQr: !!pixData.encodedImage });
            }
          }
        }
      }
    } catch (pixErr) {
      log("Warning: could not fetch first payment PIX data", { error: String(pixErr) });
    }
    ...
    // Save first payment record if available
    if (firstPayment.id) {
      await supabase.from("asaas_payments").insert({
        user_id: user.id,
        asaas_id: firstPayment.id,
    ...
    return new Response(JSON.stringify({
      asaas_subscription_id: asaasSub.id,
      first_payment_id: firstPayment.id || null,
      plan_code: plan_code,
      plan_name: plan.name,
      status: "ACTIVE",
      next_due_date: nextDueDate,
