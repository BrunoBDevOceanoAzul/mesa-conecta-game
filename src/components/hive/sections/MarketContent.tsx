/**
 * @file MarketContent.tsx
 * @description Seção Market - Mercado de Mesas e Serviços
 * @module components/hive/sections/MarketContent
 */

'use client';

import React from 'react';
import { motion } from 'framer-motion';
import { ShoppingBag, Search } from 'lucide-react';

export default function MarketContent() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0612] via-[#0f0a1a] to-[#050505] text-white p-4 pb-24 md:p-8">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="max-w-4xl mx-auto"
      >
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold font-display flex items-center gap-2">
            <ShoppingBag className="w-6 h-6 text-[#D94367]" />
            Mercado
          </h1>
          <button className="p-2 bg-white/5 rounded-xl border border-white/10">
            <Search className="w-5 h-5" />
          </button>
        </div>
        
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 text-center">
          <ShoppingBag className="w-12 h-12 text-white/20 mx-auto mb-4" />
          <h3 className="text-lg font-semibold mb-2">Mercado de Mesas</h3>
          <p className="text-white/50 text-sm max-w-md mx-auto">
            Descubra mesas disponíveis, encontre mestres e reserve sua próxima aventura.
          </p>
          <a 
            href="/explorar" 
            className="inline-block mt-4 px-6 py-2 bg-[#662583] rounded-xl text-sm font-medium hover:bg-[#662583]/80 transition-colors"
          >
            Explorar Mesas
          </a>
        </div>
      </motion.div>
    </div>
  );
}
